...
DATA_PATH = PROJECT_DIR / "delivery.csv"
OUTPUT_PATH = PROJECT_DIR / "delivery_grade_results.csv"
delivery_df = pd.read_csv(DATA_PATH)
...
delivery_df["label"] = delivery_df["label"].astype(int)
...
print(delivery_df["label"].value_counts())


# 3. 设置特征和目标变量
X = delivery_df["review"]
y = delivery_df["label"]
